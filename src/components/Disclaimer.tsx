...
const sourceFilePath = import.meta.env.VITE_OPENAIR_SOURCE_FILE

export function Attribution() {
  return (
    <Paper  sx={{ p:2, alignItems: 'flex-end'}}>
      <Typography variant="h6" color="text.secondary" sx={{ mx: 'auto', width: 200 }}>
        Open Air source data file: 
      </Typography>
      <Typography  sx={{ mx: 'auto', width: 200 }}>
        {sourceFilePath.split('/')[sourceFilePath.split('/').length - 1]} 
      </Typography>
      <Divider sx={{ my: 2 }}/> 
      <Typography variant="body2" color="text.secondary" sx={{ mx: 'auto', width: 200 }}>
